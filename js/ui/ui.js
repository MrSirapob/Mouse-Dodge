import { CONFIG } from "../core/config.js?v=20260829-rlfe";
import { RARITY_CONFIG, RARITY_ORDER, SKINS, SKINS_BY_RARITY } from "../data/skins.js?v=20260829-rlfe";
import { tick as playReelTick } from "../audio/reelTick.js?v=20260829-rlfe";
import { mountSkinCanvas, mountSkinCanvases } from "../rendering/skinPreview.js?v=20260829-rlfe";
...
export class UI {
  ...
  cacheElements() {
    ...
    this.p1SkillPicker = document.getElementById("p1SkillPicker");
    ...
    this.skillNextBtn = document.getElementById("skillNextBtn");
  ...
  bindMenu() {
    ...
    document
      ...
      ?.addEventListener("click", () => {
        if (this.currentMode === "coop" && this.coopSkillStep === 2) {
          this.goToP1SkillStep();
        } else {
          this.showModeScreen();
        }
      });
    document
      .getElementById("skillNextBtn")
      ?.addEventListener("click", () => this.goToP2SkillStep());
    document
  ...
  showSkillScreen() {
    ...
    // Coop always restarts the loadout flow on P1's step: pick P1's skill,
    // "ถัดไป" -> pick P2's skill, then "เริ่มเล่น" appears.
    this.coopSkillStep = 1;
  ...
  renderSkillStep() {
    ...
    const onP2Step = coop && this.coopSkillStep === 2;

    this.p1SkillPicker?.classList.toggle("hidden", onP2Step);
    this.p2SkillPicker?.classList.toggle("hidden", !onP2Step);

    if (this.skillScreenSub) {
      this.skillScreenSub.textContent = !coop
        ? "เลือกสกิลที่ต้องการก่อนเริ่มเกม"
        : onP2Step
          ? "PLAYER 2 เลือกสกิลที่ต้องการ"
          : "PLAYER 1 เลือกสกิลที่ต้องการ";
    }
    if (this.controlHint) {
    ...
    // In coop, P1's step shows "ถัดไป" instead of "เริ่มเล่น" — the run
    // can't start until P2 has picked a skill too.
    const showNext = coop && !onP2Step;
    this.skillNextBtn?.classList.toggle("hidden", !showNext);
    this.startBtn?.classList.toggle("hidden", showNext);
  }

  goToP2SkillStep() {
    if (this.currentMode !== "coop") return;
    this.coopSkillStep = 2;
    this.renderSkillStep();
  }
