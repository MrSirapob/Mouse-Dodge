...
export async function capturePatternPlan(n) {
  ...
  const durationFor = {
    ...
    voidWell: (args) => (args[1] - 1) * (args[2] ?? 0.22),
    voidPulse: (args) => Math.max(0, (args[1] - 1) * (args[2] ?? 2.2)),
    gravityRain: (args) => (args[1] - 1) * (args[2] ?? 0.18),
    gravityFlip: (args) => (args[1] - 1) * (args[2] ?? 0.16) + (args[4] ?? 1.0),
    gravityCross: (args) => (args[1] - 1) * (args[2] ?? 2.2),
    shadowEcho: (args) => args[1] ?? 1,
    shadowTrail: (args) => (args[1] - 1) * (args[2] ?? 0.18),
    shadowCross: (args) => (args[1] - 1) * (args[2] ?? 0.2),
    closingLanes: (args) => (args[1] - 1) * (args[2] ?? 2.1),
    collapseCross: (args) => (args[1] - 1) * (args[2] ?? 2.4),
    movingSafeGap: (args) => (args[1] - 1) * (args[2] ?? 2.0),
    voidLane: (args) => (args[1] - 1) * (args[2] ?? 1.0),
    voidCollapse: (args) => (args[1] - 1) * (args[2] ?? 1.0),
    voidSplit: (args) => (args[1] - 1) * (args[2] ?? 1.15),
    voidBlackout: (args) => (args[1] - 1) * (args[2] ?? 1.0),
    gravitySnap: (args) => (args[1] - 1) * (args[2] ?? 0.9),
    gravityExchange: (args) => (args[1] - 1) * (args[2] ?? 0.85),
    gravitySnapLine: (args) => (args[1] - 1) * (args[2] ?? 0.9),
    gravityWellChain: (args) => (args[1] - 1) * (args[2] ?? 0.75),
    shadowFreeze: (args) => (args[1] - 1) * (args[2] ?? 0.9),
    shadowChase: (args) => (args[1] - 1) * (args[2] ?? 0.08),
    shadowMemory: (args) => (args[1] - 1) * (args[2] ?? 0.8),
    shadowMirror: (args) => (args[1] - 1) * (args[2] ?? 0.85),
    collapseCurtain: (args) => (args[1] - 1) * (args[2] ?? 0.8),
    collapseCorners: (args) => (args[1] - 1) * (args[2] ?? 1.0),
    collapseCircle: (args) => (args[1] - 1) * (args[2] ?? 0.85),
    collapseChambers: (args) => (args[1] - 1) * (args[2] ?? 0.75),
    collapseSweep: (args) => (args[1] - 1) * (args[2] ?? 0.7),
    ritualRing: () => 1.1,
    ritualSeal: (args) => (args[1] - 1) * (args[2] ?? 0.8),
    judgmentLine: (args) => (args[1] - 1) * (args[2] ?? 0.75),
    judgmentCross: (args) => (args[1] - 1) * (args[2] ?? 0.85),
    ritualClock: (args) => (args[1] - 1) * (args[2] ?? 0.7),
    threeJudgments: (args) => (args[1] * 3 - 1) * (args[2] ?? 0.7),
    finalEclipse: () => 0.1,
